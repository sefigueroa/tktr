...
try:
    dfModel = pd.read_excel(inputDir + inputDataFile)
except FileNotFoundError:
    print("Error al leer el archivo ", inputDir+inputDataFile)
    exit()
...
if df_duplicados.empty:
    ...
    df_duplicados.to_excel(outputDir+"DUPLICADOS.xlsx")
...
try:
    dfConstantes = pd.read_excel(consDir + consGruposCuadrillas)
except FileNotFoundError:
    print("Error al leer el archivo ", consDir + consGruposCuadrillas)
    exit()
...
#quito del nuevo los que ya están en el anterior
#print("\n\nTickets previamente generados en Service Now:")
#print(dfSucceeded[['Número','Descripción']])


#print("\n\nTickets para generar segun PowerBI:")
#print(dfToReduce)
...
wb = openpyxl.load_workbook(outputDir + outputFileTktsMasivos)
ws = wb.active
...
wb.save(outputDir + outputFileTktsMasivos)

#Renombro los archivos procesados para evitar errores(chatGPT)
try:
    # Separa la ruta del archivo de la extensión
    filename, extension = path.splitext(inputDataFile)

    # Renombra el archivo agregando un guion bajo al final
    new_filename = '_' + filename + extension
    rename(path.join(inputDir, inputDataFile), path.join(inputDir, new_filename))
    

except Exception as e:
    print(f"Error renombrando el archivo de entrada {inputDataFile}: {e}")
    exit()
